...
my $PW = $ENV{'PASSWD'};
my $ID_RSA = "/home/$USER/.ssh/id_rsa";
my $USER_DATA_DIR = "/home/$USER/galaxy/$CONTAINER_ID";

my $REMOTE_BLAST_DB_DIR = "/home/okuda/data/db/refseq_protein/20140911";
my $BLAST_DB = "microbial.protein";
my $REMOTE_DATA_DIR = "/home/$USER/gw_dir/$CONTAINER_ID";

# $REMOTE_DATA_DIRを$GW_DATA_DIRにmountしているものとする。
...
my $SCRIPT_PREFIX = 'refseq-search';
my $SPRIT_SEQ_NUM = 100;
my $THREAD_NUM = 4;
...
sub main {
    ...
    my $cmd = "sudo -u $USER sh -c \"if [ ! -e $USER_DATA_DIR ]; then mkdir -p $USER_DATA_DIR ; fi\"";
    &check_cmd_result($cmd, "create user data dir");
    ...
    while ($j <= $total_file_count) {
        my $cmd = "sudo -u $USER sh -c \"scp -i $ID_RSA $USER\@$GW:$GW_DATA_DIR/$OUTPUT_FNAME.$j $USER_DATA_DIR/$OUTPUT_FNAME.$j\"";
        &check_cmd_result($cmd, "copy result file $j");
        `cat $USER_DATA_DIR/$OUTPUT_FNAME.$j >> $OUTPUT`;
        ++$j;
    }
...
sub split_fasta_file {
    ...
    open DATA, $INPUT or die;
    open OUT, ">$USER_DATA_DIR/$INPUT_FNAME.${SCRIPT_PREFIX}_${file_count}_${total_file_count}" or die;
...
sub create_remote_command_script {
    ...
    my $script = "$USER_DATA_DIR/$SCRIPT_PREFIX.${file_count}_${total_file_count}.sh";
...
sub scp_files {
    ...
    my $cmd1 = "sudo -u $USER sh -c \"ssh -i $ID_RSA $USER\@$GW 'if [ ! -e $GW_DATA_DIR ]; then mkdir -p $GW_DATA_DIR ; fi'\"";
    ...
    while ($i <= $total_file_count) {
        $script = "$USER_DATA_DIR/$SCRIPT_PREFIX.${i}_${total_file_count}.sh";
        $input_file = "$USER_DATA_DIR/$INPUT_FNAME.${SCRIPT_PREFIX}_${i}_${total_file_count}";

        # copy remote command script
        my $cmd2 = "sudo -u $USER sh -c \"scp -i $ID_RSA -p $script $USER\@$GW:$GW_DATA_DIR\"";
        &check_cmd_result($cmd2, "scp script file $i");

        # copy data file
        my $cmd3 = "sudo -u $USER sh -c \"scp -i $ID_RSA $input_file $USER\@$GW:$GW_DATA_DIR\"";
...
sub post_job {
    ...
    my $script = "$REMOTE_DATA_DIR/$SCRIPT_PREFIX.${file_count}_${total_file_count}.sh";
